...
rescale_function <- function(x) {
  map(names(x), ~ test[[.x]] %>% habicon::rescale_map()) %>% 
    do.call(c, .)
}
...
# save
writeRaster(patch_sum, "data/output_habicon/patch_priority_all.tif")

writeRaster(corr_sum, "data/output_habicon/corridor_priority_all.tif")
```
...
cleaned_acs <- acs %>% 
  ...
  dplyr::select(GEOID, NAME, total_population = B01003_001, housing_burden:percent_low_income) 


st_write(cleaned_acs, "shiny/data/acs_Larimer_2022.shp")
